...
class driver_assistant(geographic_agent.geographic_agent):
    ...
    def __init__(self, origin, route, battery, map, is_priority, id, time_u, price_u, distance_u):
        ...
        self.defined_velocity = np.array((0.1, 0.1))
        ...
        self.next_node= (self.lng,self.lat)
        if is_priority:
        ...
        self.map.points_to_print =[]
        #ver os nodes
        print(self.lng)

        print(get_coordinates_of_node(self.G, self.list_route[0][0]))
        # for j in self.list_route:
        #     for i in j:
        #         self.map.add_points_to_print(get_coordinates_of_node(self.G, i),'k')
    ...
    def animate(self):
        ...
        self.set_latitude(self.y)
        self.set_longitude(self.x)
    ...
    def update_current_route(self):
        ...
        x_true = np.isclose(xpath[self.next_index_node], self.x, rtol=1.0e-6)
        y_true = np.isclose(ypath[self.next_index_node], self.y, rtol=1.0e-6)
        print(x_true)
        print(y_true)

        if (len(self.current_route)>self.next_index_node or self.next_index_node  == 0) and x_true and y_true   :
            ...
            #self.next_node = np.array(self.upcoming_node_coordinates())
            self.next_node = get_coordinates_of_node(self.G,self.current_route[self.next_index_node])
            #print('next node')
            ...
            print('new node!')
            self.map.add_points_to_print(self.next_node,'m')
            self.next_index_node +=1
        ...
        #Decimal(self.next_node[0])-Decimal(position[0]),
        position = np.array((self.get_longitude(), self.get_latitude()))
        print(position)
        velocity_direction = unit_vector(self.next_node - position)
        print(velocity_direction)
        defined_velocity =[0.1,0.1]
        #velocity_direction = unit_vector((Decimal(self.next_node[0])-Decimal(position[0]),Decimal(self.next_node[1])-Decimal(position[1])))
        if (xpath[self.next_index_node] - position[0]!=0 and ypath[self.next_index_node] - position[1]!=0  ):
            self.velocity = velocity_direction * defined_velocity #* self.velocity
        else:
        ...
        print(self.get_latitude())
        print('point_pos')
        print(xpath[self.next_index_node])
        print(ypath[self.next_index_node])
        print('vel')
        print(self.velocity)
        print('********')
...
#
    def crossed_node_event(self):
        ...
        """
        car_near_xnode = np.isclose(self.car_view[0][0], self.x, rtol=1.0e-6)#estava a 6
